                        ...
                        <div class="text-center mb-4">
                            <div class="position-relative d-inline-block">
                                @if($customer->profile_image)
                                    <img src="{{ public_storage_url($customer->profile_image) }}" alt="Current profile photo" class="img-fluid profile-preview" style="width: 250px; height: 250px;border-radius: 8px; border: 3px solid #dc3545; background-color: #f8f9fa;">
                                @else
...
function previewImage(input) {
    ...
    if (file) {
        ...
        const img = document.querySelector('.profile-preview');
        if (img) {
            img.src = "{{ public_storage_url($customer->profile_image) }}";
        } else {
            const initialsDiv = document.getElementById('initials-div');
            if (initialsDiv) {
